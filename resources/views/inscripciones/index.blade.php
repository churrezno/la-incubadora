...
@section('content')
    ...
    <x-adminlte-datatable id="tableInscripciones" :heads="$heads" :config="$config">
    ...
    <script>
        ...
        function updateAsignaciones(event) {
            ...
            let table = $('#tableInscripciones').DataTable( {
                ...
                // Store the state of opened child rows before reloading the data
                // table.rows().every(function () {
                //     var row = this;

                //     if (row.child.isShown()) {
                //         openedChildRows.push(row.index());
                //     }
                // });
            ...
            $.ajax({
                ...
                success: function(){
                    ...
                    //     // Restore the state of opened child rows after the reload
                    //     openedChildRows.forEach(function (index) {
        ...
        function updateCategoria(event, inscripcionId) {
            event.preventDefault();
            let table = $('#tableInscripciones').DataTable( {
                            retrieve: true,
                        }),
            ...
            $.ajax({
                url: '/admin/inscripciones/update-category/' + inscripcionId,
                type: 'POST',
                processData: false,
                contentType: false,
                data: formData,
        ...
        function removeInscripcion(id) {
            ...
            Swal.fire({
                title: "¿Seguro que quieres eliminar la inscripción con id = " + id + "?",
